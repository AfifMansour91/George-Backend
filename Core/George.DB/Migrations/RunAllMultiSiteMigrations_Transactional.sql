-- ============================================================================
-- MultiSite Phase 2 - RUN-ALL, TRANSACTION-WRAPPED variant.
-- Identical content to RunAllMultiSiteMigrations.sql, wrapped in ONE transaction
-- for all-or-nothing safety:
--   * SET XACT_ABORT ON  -> any runtime error aborts the batch and rolls back.
--   * After every GO, "IF XACT_STATE() = 0 SET NOEXEC ON" stops all later steps
--     once the transaction has rolled back, so you never get a partial install.
-- Still fully idempotent + self-repairing: safe to re-run.
--
--   * Recommended run: sqlcmd -b -i RunAllMultiSiteMigrations_Transactional.sql
--     (also fine in SSMS).
--   * If a run DID error: everything was rolled back. The session may be left with
--     NOEXEC ON -> run "SET NOEXEC OFF;" (or open a new window) before retrying.
--
-- Prefer this file when you want atomic apply; the plain RunAllMultiSiteMigrations.sql
-- remains the canonical step-by-step reference.
-- ============================================================================
...
SET NOEXEC OFF;
PRINT N'MultiSite Phase 2 (transactional) install committed successfully.';
GO
